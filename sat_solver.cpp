...
void verify_solution(SAT_Manager mng) {
    int num_verified = 0;
    for (int cl_idx = SAT_GetFirstClause (mng); cl_idx >= 0; cl_idx = SAT_GetNextClause(mng, cl_idx)) {
        ...
        for (i = 0; i < len; ++i) {
            ...
            if ((var_value == 1 && sign == 0) || (var_value == 0 && sign == 1)) break;
        }
        if (i >= len) {
...
int main(int argc, char ** argv) {
    ...
    while (++current < argc) {
        switch (argv[current][1]) {
            case 't':
                SAT_SetTimeLimit(mng, atoi(argv[++current]));
                break;
            case 'c':
                SAT_SetCacheSize(mng, atoi(argv[++current]));
                break;
            // case 'e':
            //     SAT_SetMaxEntry(mng, atoi(argv[++current]));
            //     break;
            // case 'm':
            //     SAT_SetMaxDistance(mng, atoi(argv[++current]));
            //     break;
            // case 'd':
            //     SAT_SetDynamicHeuristic(mng, true);
            //     break;
            case 's':
                SAT_SetStaticHeuristic(mng, true); // false -> true
                static_heuristic = true;
                static_argv_index = ++current; // record the place of static ordering input file
                break;
            case 'h':
                option = atoi(argv[++current]);
                ...
                break;
            case 'n':
                SAT_SetMaxNumLearnedClause(mng, atoi(argv[++current]));
                break;
            case 'o':
                SAT_SetOldestEntry(mng, atoi(argv[++current]));
                break;
            case 'l':
                SAT_SetCleanLimit(mng, atoi(argv[++current]));
                break;
            case 'r':
                SAT_SetCrossFlag(mng, false); // true -> false
                break;
            case 'a':
                SAT_SetAdjustFlag(mng, false); // true -> false
                break;
            case 'b':
                SAT_SetBacktrackFactor(mng, atof(argv[++current]));
                break;
            case 'f':
                SAT_SetFarBacktrackFlag(mng, true);
                break;
            case 'q':
                SAT_SetQuietFlag(mng, true); // true -> false
                quiet = true;
                break;
            default:
                cout << "unkonwn option! " << argv[current] << endl;
                exit(0);
        } // end switch
    }
